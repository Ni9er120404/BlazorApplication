...
namespace BlazorApplication.Server.Services.ProductService
{
	public class ProductService : IProductService
	{
		...
		{
			Product? products = await _context.Products.FirstOrDefaultAsync(product => product.Id == id);
		...
		public async Task<List<Product>> GetProductsByCategory(string categoryUrl)
		{
			...
			List<Product>? products = await _context.Products.Where(product => product.CategoryId == category.Id).ToListAsync();
